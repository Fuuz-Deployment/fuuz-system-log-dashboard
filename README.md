# Fuuz System Log Dashboard â Reference Guide

> **Audience:** Developers, Managers, and Administrators  
> **Version:** 0.0.9 Â· Deploy Date: 2026-06-14  
...
The **System Log Dashboard** is a multi-tab observability and governance screen built in Fuuz. It provides a unified view of the health, activity, and risk posture of every major artifact type in a Fuuz environment â users, screens, data flows, data models, documents, saved scripts, saved APIs â plus two special-purpose tools (an embedded HTML governance report and an interactive ERD).

Each tab follows the same structure:

1. **Stat cards (KPI bar)** â headline metrics at a glance, color-coded by urgency (blue = informational, green = healthy, orange = warning, red = critical).
2. **Chart panels** â interactive charts that expand to full-screen via the âº icon in the top-right corner of each card.
3. **Detail tables** (on some tabs) â filterable row-level lists of the specific items that contribute to a flagged KPI.
...
| **Authentication Trend by Domain** | Multi-series line + avg line | Watch for new domains appearing or existing ones dropping to zero â both indicate permission or integration changes. The Fuuz Team line dominates; any sustained orange (fanucamerica.com) or green (40solutions.com) spike warrants review. |
...
**Purpose:** Monitor the health of the screen inventory â which screens are deployed, which carry risk, and where element hygiene needs attention.
...
| STALE 90D+ | 175 | Screens not touched in 90+ days | High value â consider archiving |
| ACTIVE UNDEPLOYED | 121 | Screens marked active but never shipped | Should be resolved or demoted |
| UPDATED 7D | 1 | Screens touched in the last 7 days (3 in 30d) | Low activity â expected in stable phase |
...
| **Edge vs Standard** (donut) | Only 1 Edge screen of 182 â Edge screens run client-side and have stricter performance requirements. |
...
| **Element Density Distribution** (histogram) | Most screens fall in the 1â10 and 26â50 element buckets. 20 screens have 100+ elements â review these for performance. |
...
| **Authorship by Domain** (bar) | Fuuz Team has authored 353 screen versions and created 182 screens â all authorship concentrated in one domain, expected for a single-org environment. |
...
| **Top Action-Heavy Screens** (bar) | entAdminHomepageDev3 has 19 action elements â the only screen above the 10-action threshold. High action counts increase flow invocation risk. |
...
Columns: Tier Â· Screen Â· Module Â· Risk Signals Â· Elements Â· Active
...
**SCRIPT-HEAVY SCREENS (Tier 1):** No screens match â â This is a green signal; no inline scripts detected.

**ACTION-HEAVY SCREENS (>10 Actions)** (1 row): entAdminHomepageDev3 â 19 action elements, 203 total elements.

**SCREENS WITH ELEMENT HYGIENE ISSUES** (9 rows)
Columns: Screen Â· Module Â· Issues Â· Detail â Lists the same screens as Risk-Bearing with human-readable issue descriptions (e.g., "37 no dataPath, 37 not in form, 40 theme-unsafe").

**BLOATED SCREENS (>50 Elements)** (25 rows): UpdatedEntAdminPage (544), xEntDash1 (449), System Log Dashboard (251), Gateway Dashboard (248), entAdminHomepageDev3 (203)â¦

### Risk signal glossary
| Signal | Meaning |
|--------|---------|
| `noDataPath` | Element has no bound data path â will render empty |
...
**Purpose:** Audit the operational health and risk posture of all data flows â their deployment state, node composition, concurrency patterns, and which flows carry elevated risk.
...
| DEPLOYMENT RATE | 80% | 32 deployed; 8 never deployed | Good â 8 undeployed flows need review |
...
| STALE 90D+ | 38 | Flows not updated in 90+ days | 95% of inventory â very high |
...
| Chart | What to look for |
|-------|-----------------|
| **Active vs Inactive** (donut) | 100% Active â all 40 flows are active. No flows have been deactivated. |
...
| **Flows by Module** (horizontal bar) | Machine Data App (19), Testing (2), Configuration (2), Orchestration (1)â¦ Machine Data App dominates â review its flows for the Tier 1 risk. |
...
| **Update Activity (Last 30 Days)** | 2 flows updated this month â very low. |

### Detail Tables

**RISK-BEARING FLOWS** (30 rows)
Columns: Tier Â· Sec Â· Flow Â· Module Â· Signals Â· Used By Â· Active
...
> **Screen Generator** is used by 70 screens â any failure or change here has wide blast radius.
...
| STALE 90D+ | 60 | Models not touched in 90 days | 97% of inventory â expected for stable schema |
...
| Chart | What to look for |
|-------|-----------------|
| **Model Kind** (donut) | 100% Reference â all models are Reference type. No Transactional-only models in this set. |
...
| **Deployment Status** (donut) | 97% Deployed (60/62). 2 models never deployed â confirm whether they are in active development or orphaned. |
...
| **Field Type Distribution** | String (2), Relation (2) â shown for the small subset of custom-typed fields. |
...
| **Largest Models (by field count)** | Diff (7 fields), DiffGroup (5 fields) â these are internal diff/versioning models with minimal fields. |
...
| **Deployment Status** | 0% Deployed â the one document has never been deployed. If it is meant for production, deploy it or change its status to inactive. |
| **Risk Tier Distribution** | 1 at risk â classified as Tier 3 (Hygiene). Not critical but should be reviewed. |
| **Render Volume Distribution** | Documents bucketed by lifetime render count â with 0 deployments, render count is 0. |

---

## Tab 6 â SAVED SCRIPTS
...
| Chart | What to look for |
|-------|-----------------|
| **Active vs Inactive** | 100% active â no retired scripts. |
...
| **Authorship by Domain** | 100% Fuuz Team â all scripts authored internally. |
...
| EMPTY QUERIES | 2 | Queries with no queryText | Fix â will return no data |
| HARDCODED LIMITS | 1 | `first: N` not parameterized | Fix â prevents dynamic pagination |
| PAGINATION BROKEN | 1 | Query missing `total` field | Fix â paginators won't know page count |
...
| Chart | What to look for |
|-------|-----------------|
| **Operation Type Mix** | 100% Query â no mutations or subscriptions saved. |
| **Risk Tier Distribution** | 4 at risk: 18.18% T1 (Broken â won't execute), 18.18% T2 (Quality issues), 63.64% No Risk. Two broken queries need immediate fix. |
| **Iteration Pattern** | 5 queries have only 1 commit (written once, never revised). 3 have 2â3 commits. High iteration suggests refinement; single commits suggest set-and-forget. |
| **Parameter Count Distribution** | Most queries have 0 parameters â they are hardcoded, making them fragile to data changes. |
...
**Left sidebar:**
- Header: `FUUZ / Screen Governance` â 182 screens scanned (as of scan timestamp)
- **Sections** (navigation links):
  - Overview â summary metrics
  - Risk (9) â screens with risk signals
  - Element Hygiene (280) â all hygiene violations
  - Activity â update/creation activity
  - Inventory â full screen list
  - Drift (298) â screens that have diverged from expected structure
- **Filter Tables:** All Â· My screens Â· Recent (7d) Â· At risk Â· Active+Undeployed
- **Layout toggle:** Force | Layered (for ERD/graph views within the report)

**Main content area:** Mirrors the SCREENS tab KPI bar and chart panels but in a richer HTML format with better typography. The "Drift" count of 298 is notable â it represents elements or configurations that have drifted from baseline expectations and is not directly surfaced on the SCREENS tab chart.

### When to use vs the SCREENS tab
Use the **SCREENS tab** for a quick at-a-glance review. Use **SCREEN HTML** when you need to filter by ownership (`My screens`), investigate recent changes, or dig into drift details â it provides the same data with more interactive filtering.
...
| Element | Description |
|---------|-------------|
| **Header stats** | 350 models Â· 571 relations Â· 28 inferred relations |
| **Search box** | Search models by name, description, or field name |
| **Model sidebar** | 346 models listed alphabetically â click any to show its 1-hop relation diagram |
| **Canvas** | Interactive graph area â scroll to zoom, drag to pan, Enter to jump to first search match |
...
| **"View complete ERD"** button | Renders all 350 models and 571 relations simultaneously â use with caution in large environments |
...
- For onboarding new developers â the complete ERD provides a map of the full schema.
...
| Pattern | Tabs | Action |
|---------|------|--------|
| Low deployment rate + high stale count | SCREENS, DATA FLOWS, DATA MODELS, DOCUMENTS | Run a cleanup sprint â archive or deploy stale artifacts |
...
- **Last Authenticated At:** `06/12/2026 7:50 AM` â the last time the current user's session was renewed
- **Screen Version:** `0.0.9` â the version of this dashboard screen definition
- **Deploy Date:** `06/14/2026 8:30 AM` â when this version was deployed to this environment
- **Role context:** Supervisor Manager Â· Fuuz Overflow Â· Build
...
| File | Tab |
|------|-----|
| `01-users-top.png` | USERS â top (auth charts) |
| `02-users-bottom.png` | USERS â bottom (change charts) |
| `03-screens-top.png` | SCREENS â KPI bar + donut charts |
| `04-screens-mid.png` | SCREENS â density + bloated + trend |
| `05-screens-deployment.png` | SCREENS â deployment health + complexity |
| `06-screens-authors.png` | SCREENS â activity + action-heavy + authors |
| `07-screens-risk-table.png` | SCREENS â risk-bearing table |
| `08-screens-hygiene-table.png` | SCREENS â hygiene + bloated tables |
| `09-dataflows-top.png` | DATA FLOWS â KPI bar + donut charts |
| `10-dataflows-nodes.png` | DATA FLOWS â node mix + modules + integrity |
| `11-dataflows-authors.png` | DATA FLOWS â module group + authors + activity |
| `12-dataflows-risk-table.png` | DATA FLOWS â risk-bearing flows table |
| `13-datamodels-top.png` | DATA MODELS â KPI bar + donut charts |
| `14-datamodels-bottom.png` | DATA MODELS â field + module + author charts |
| `15-documents.png` | DOCUMENTS â full tab |
| `16-savedscripts.png` | SAVED SCRIPTS â full tab |
| `17-savedapis-top.png` | SAVED APIS â KPI bar + charts |
| `18-screenhtml-top.png` | SCREEN HTML â embedded governance report |
| `19-screenhtml-sidebar.png` | SCREEN HTML â sidebar + overview detail |
| `20-fuuzerd.png` | FUUZ ERD â interactive model diagram |


---

## ⚠️ Known Limitation: `sensitive` Field Flag (Build Only — Not Yet in QA/Prod)

**As of June 15, 2026**, a new `sensitive` flag has been added to data model fields in the **build** environment. This feature is **not yet available in QA or Production** and is expected to roll out in the coming weeks.

### What It Does

The `sensitive` flag can be set on individual data model fields. During the Fuuz package export process, the platform checks for this flag and **excludes sensitive fields from exported packages**, preventing accidental inclusion of sensitive data in package definitions.

### Impact on This Dashboard

Because the System Log Dashboard queries data model field metadata (including the `sensitive` property), running the data model graph query in **QA or Production environments may produce an error** — since those environments do not yet have the `sensitive` field on the data model schema.

### Workaround

1. Open the **Data Model** flow in your environment's flow editor.
2. Locate the GraphQL query node that retrieves field metadata.
3. Remove the `sensitive` field from the query's field selection.
4. **Save and redeploy** the flow.

This will resolve the error until the `sensitive` flag is promoted to QA and Production.
